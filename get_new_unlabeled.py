...
if __name__ == '__main__':
    location = './unlabeled'
    print(f'Running through video at {sys.argv[1]}, finding all bees and putting them in {location}.')
    ...
    while frame is not None:
        ...
        for bee in bees:
            cv2.imwrite(os.path.join(location, f'{os.path.basename(sys.argv[1])}bee{bee[1]}.png'), bee[0])
        _, frame = cap.read()
        if show:
